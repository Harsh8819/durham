...
int main()
{
	try
	{
		 ...
		 *	DECLARATIONS
		 ************************/
		const string START_POINT_LABEL = "AA"; // The label of the start point
		map<string, Vector2D<double>> pointsMap;
		map<string, Vector2D<double>>::iterator mapIter ; // an iterator to traverse the map

		string dataFileName = "MockDataForTesting.txt";
		double totalDistance = 0;	

		string theLabel; // a temporary string to hold a label
		Vector2D<double> thePoint;
		Vector2D<double> pointFrom;
		
		ifstream in; // input file stream for the data file.
		in.open(dataFileName.c_str());
		if (in.is_open())
		{

			while (in>>theLabel>>thePoint)
			{				
				pointsMap[theLabel] = thePoint;			
			}			
			for (auto mapIter : pointsMap)
			{
	 ...
	 ******************************************************************************/
	catch (const exception& ex) // an exception was thrown
	{
		cerr << "\nAn error occurred : " << ex.what() << endl;
	}
